...
#ambloc=LPM.LSCM()
#ambloc=LPM.MLCP(29)
#ambloc=LPM.DSM(29,0.95)
#ambloc=LPM.MEXCLP(29,0.75)
...
"""
alpha=1
filepath = os.path.join(r'C:\Users\ffede\Google Drive\Reading_\Figure','Vecchie.txt')
data = pd.read_csv(filepath, sep=" ", header = None)
ambloc=data[1]
...
def AmbulanceManager1(A,env,e):
    ...
    #print("Manager 1")
    if A.availability==1:
        #print(A.position,e.zone)
        if COP1[A.position][e.zone]==1:
            success=1
            A.dislocation(env,e)
...
class Global_vars:
    ...
    for i in range(len(ambloc)):
        ...
        if ambloc[i]>=1:
            for j in range(int(ambloc[i])):
                it=Ambulance(len(ambulances)+1,i)
