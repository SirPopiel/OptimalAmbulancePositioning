function ExpCover(x,y,ambloc,exp)
close all;
...
% expcov=z/sum(z);
% Color=(0.7-120*expcov)+0.3;
% Color2=0*expcov+0.15;
% Color3=120*expcov+0.3;
expcov=z;
%maxval=max(expcov);
maxval=4.2;
Color=1-expcov/maxval;
Color2=0*expcov+0.15;
Color3=expcov/maxval;
...
axis square
title('Copertura attesa territorio');
    ...
    for i=1504000:halfRaggio2:1522000    
        ...
        if((a==1)||(b==1))
         ...
         if(numeros(b,a)>0)
          ...
          if (copdop(b,a)==1) 
                  ...
                  ,'FaceColor',[Color(k) Color2(k) Color3(k)]);
            if ambloc(k,2)==1
                  plot(i+0.5*halfRaggio2,j-0.5*halfRaggio2,'yo')
            end
            if ambloc(k,2)==2
                  plot(i+0.5*halfRaggio2,j-0.5*halfRaggio2,'ko')
            end
            if ambloc(k,2)>=3
                  plot(i+0.5*halfRaggio2,j-0.5*halfRaggio2,'ro')
            end   
          k=k+1;
        ...
        elseif((numeros(b,a)>0)||((numeros(b-1,a)>0)&&(numeros(b,a-1)>0)))
          ...
          if (copdop(b,a)==1) 
                  ...
                  ,'FaceColor',[Color(k) Color2(k) Color3(k)]); 
          if ambloc(k,2)==1
                  plot(i+0.5*halfRaggio2,j-0.5*halfRaggio2,'go')
          end
          if ambloc(k,2)==2
                  plot(i+0.5*halfRaggio2,j-0.5*halfRaggio2,'ko')
          end
...
a=[1514973, 5034540];
scatter(a(1), a(2),'wo','filled')
t=text(a(1)+200, a(2)+200,'Duomo');
t.Color=[0.95 0.95 0.95];

a=[1517885, 5036285];
scatter(a(1), a(2),'wo','filled')
t=text(a(1)+200, a(2)+200,'PoliMi');
t.Color=[0.95 0.95 0.95];
...
a=[1509698, 5036065];
scatter(a(1), a(2),'wo','filled')
t=text(a(1)+300, a(2)+300,'San Siro');
t.Color=[0.95 0.95 0.95];

a=[1516264, 5040667];
scatter(a(1), a(2),'wo','filled')
t=text(a(1)+300, a(2)+300,'Bicocca')
t.Color=[0.95 0.95 0.95];
...
a=[1517540, 5031847];
scatter(a(1), a(2),'wo','filled')
t=text(a(1)+300, a(2)+300,'Corvetto');
t.Color=[0.95 0.95 0.95];
...
a=[1512444, 5039122];
scatter(a(1), a(2),'wo','filled')
t=text(a(1)+300, a(2)+300,'Bovisa');
t.Color=[0.95 0.95 0.95];

a=[1510691, 5040336];
scatter(a(1), a(2),'wo','filled')
t=text(a(1)+300, a(2)+300,'Quarto Oggiaro');
t.Color=[0.95 0.95 0.95];
...
a=[1516055, 5037079];
scatter(a(1), a(2),'wo','filled')
t=text(a(1)+300, a(2)+300,'Centrale');
t.Color=[0.95 0.95 0.95];

a=[1511125, 5032822];
scatter(a(1), a(2),'wo','filled')
t=text(a(1)+300, a(2)+300,'Giambellino');
t.Color=[0.95 0.95 0.95];

cbr=colorbar;
changecolormapgredtoblue;
set( cbr, 'YTick', 0:0.25:1.75 );
caxis([0 1.75])